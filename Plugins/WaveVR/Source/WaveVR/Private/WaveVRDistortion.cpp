	...
	for (uint32 y = 0; y < DistortionPointsY; ++y)
	{
		for (uint32 x = 0; x < DistortionPointsX; ++x)
		{
			...
			for (uint32 i = 0; i < 10; ++i)
			{
				...
				//UE_LOG(LogHMD, Log, TEXT("Approx. %d:  input(%f, %f) G(%f,%f)"), i,
				//		UnDistortedCoord.X, UnDistortedCoord.Y, DistortedCoords[1].X, DistortedCoords[1].Y);
...
void FWaveVRHMD::DrawDistortionMesh_RenderThread(struct FRenderingCompositePassContext& Context, const FIntPoint& TextureSize)
{
	...
	//UE_LOG(LogHMD, Log, TEXT("StereoPass: %d ViewportSize (%d, %d) TextureSize (%d, %d)"), (int)View.StereoPass, ViewportSize.X, ViewportSize.Y, TextureSize.X, TextureSize.Y);
